...
#' = "CC"} the \code{scoreNARCC} function will accept and score either 7 or 10
#' CC items, although the 7-item version is recommended.  The NAR scale has 6
#' items, and the \code{scoreNARCC} function will accept only 6 NAR items when
#' \code{whichScale = "NAR"}.
#'
#' If you want to score both the CC and NAR scales, then you need to run the
#' \code{scoreNARCC} function twice, once for CC and again for NAR.
#'
#' @note
#' The \code{scoreNARCC} function assumes that your item data are numerically
...
#' this is the case, you MUST let the \code{scoreNARCC} function know this by
...
#' number of valid item responses on the scale for each respondent.
#'
#' @references
